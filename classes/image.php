<?php namespace Inerba\RemoteImage\Classes;

use Inerba\RemoteImage\Models\Settings;
...
class Image
{
    ...
    {
        $remote_file = strtok($this->urlPath,'?');
        $temp_file = storage_path('temp/') . md5($remote_file) . '.' . pathinfo($remote_file, PATHINFO_EXTENSION);
    ...
    public function temp_file_name()
    {
        $remote_file = strtok($this->filePath,'?');
        return storage_path('temp/') . md5($remote_file) . '.' . pathinfo($remote_file, PATHINFO_EXTENSION);
    }

    public function file_path_temp($width, $height)
    {
        return storage_path('app/uploads/public/'.substr(chunk_split($this->diskName(),3,'/'),0,11).'/'.$this->getThumbFilename($width, $height));
    }

    public function path_temp($disk_path)
    {
        return storage_path('app/uploads/public/'.$disk_path);
    }

    public function disc_temp_name()
    {
        return substr(chunk_split($this->diskName(),3,'/'),0,11).'/';
    }
    ...
    public function resize($width = false, $height = false, $options = [])
    {
        ...
        $this->filePath = $this->temp_file_name();
        ...
        if (!is_file($this->file_path_temp($width, $height))) {
            $this->filePath = $this->temp_file();
            $copied = true;
        }
        ...
        if (! $this->isImageCached()) {
            ...
            if(isset($copied)){
                ...
                \File::delete(storage_path('app/uploads/public/'.$this->disc_temp_name().$this->diskName()));
            }
